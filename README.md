...
RK StudyMind is a fully offline AI study app that lets you chat with your lecture notes, generate flashcards with score tracking, take multiple choice quizzes, and generate structured mindmaps — all running locally on your PC using LM Studio. No internet, no API keys, no subscriptions.
...
| 📝 **Smart Quiz** | Auto-generates multiple choice questions (A–D) with a progress bar, live score pill, inline answer selection, and a final results screen. Supports up to 40 questions with batch generation. Context limits adapt to the detected model's context window. |
| 🃏 **Flashcards** | Generates up to 40 Q&A study cards. One card at a time with ✅ / ❌ score tracking, a live score bar, and a session results screen. Deduplication pass removes repeat cards. |
...
| Document Reading | PyMuPDF 1.25.5 (PDF) · python-docx (DOCX) · python-pptx (PPTX) · ebooklib + BeautifulSoup (EPUB) · built-in (TXT, MD) |
...
| Math Rendering | `modules/math_renderer.py` — two-pass LaTeX + plain-text pipeline |
...
│   ├── math_renderer.py        # Two-pass math rendering (LaTeX + plain-text patterns)
│   ├── study_history.py        # Quiz session history and flashcard performance tracking
│   ├── exporters.py            # CSV and study report export helpers
│   └── __init__.py
├── data/                       # ChromaDB vector store + library metadata (JSON) + engine config
...
- **Quiz** — up to 40 questions, batch generation, 3-strategy parser (numbered / table / loose), adaptive context limit, running score
- **Flashcards** — up to 40 cards, deduplication, adaptive context limit, session results screen
